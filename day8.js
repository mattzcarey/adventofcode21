const fullInput = `cgdf eagcbf fc adefg eacdb fbedga geafcd efc dacfe fdgaecb | dcefbag dgcf fc daefc
...
let notes = fullInput
  ...
  .map((line) => {
    ...
    signals = signals.split(' ').map((signal) => signal.split('').sort().join(''));
...
for (let line of notes) {
    let matches = line.outputs.filter(v => [2,3,4,7].includes(v.length))
